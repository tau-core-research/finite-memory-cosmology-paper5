#!/usr/bin/env python3
"""Validate TCCS object preflight."""
...
MATRIX = ROOT / "evidence/p_taucov_tccs_object_preflight_matrix.csv"
GATES = ROOT / "evidence/p_taucov_tccs_object_preflight_gates.csv"
SUMMARY = ROOT / "evidence/p_taucov_tccs_object_preflight_summary.csv"
DOC = ROOT / "docs/p_taucov_tccs_object_preflight.md"
OUT = ROOT / "evidence/p_taucov_tccs_object_preflight_validation.csv"

AUDIT_ID = "P_TAUCOV_TCCS_OBJECT_PREFLIGHT_VALIDATION"
ALLOWED_STATUSES = {
...
def main() -> int:
    ...
    if not all(path.exists() for path in [MATRIX, GATES, SUMMARY, DOC]):
        pd.DataFrame(rows).to_csv(OUT, index=False)
        print("P_TAUCOV_TCCS_OBJECT_PREFLIGHT_INVALID")
        return 1
    ...
    print("P_TAUCOV_TCCS_OBJECT_PREFLIGHT_VALID" if ok else "P_TAUCOV_TCCS_OBJECT_PREFLIGHT_INVALID")
