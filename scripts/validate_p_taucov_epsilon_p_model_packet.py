#!/usr/bin/env python3
"""Validate the target-blind epsilon_P P-TauCov model packet."""
...
OUT = ROOT / "evidence/p_taucov_epsilon_p_model_packet_validation.csv"
...
def main() -> int:
    ...
    def add(check_id: str, passed: bool, required: bool = True) -> None:
        records.append(
            {
                "AuditID": "P_TAUCOV_EPSILON_P_MODEL_PACKET_VALIDATION",
                "CheckID": check_id,
                "Passed": bool(passed),
    ...
    if not all(path.exists() for path in required_paths):
        ...
        print("P_TAUCOV_EPSILON_P_MODEL_PACKET_INVALID")
        return 1
    ...
    add("model_family_epsilon_p", manifest.get("ModelFamily") == "epsilon_P_minimal_projection_response")
    add("lambda_B_zero", manifest.get("lambda_B") == 0)
    add("epsilon_P_one", manifest.get("epsilon_P") == 1)
    add("strict_linear_failed", manifest.get("StrictLinearParentStatus") == "FAIL_STRICT_LINEAR_REJECTED")
    add("epsilon_packet_frozen", manifest.get("EpsilonPModelPacketFrozen") is True)
    ...
    add("leakage_all_pass", leakage["Passed"].map(bool_from_csv).all())

    summary = pd.read_csv(SUMMARY)
    add("summary_packet_frozen", bool_from_csv(summary["EpsilonPModelPacketFrozen"].iloc[0]))
    ...
    print("P_TAUCOV_EPSILON_P_MODEL_PACKET_VALID_SPECIFICITY_ONLY_NO_SCORING")
    return 0
