...
D_VECTOR = DATA / "d_branch_derivative_regularized_bootstrap_smoke_reconstruction_vector.csv"
D_SAMPLES = DATA / "d_branch_derivative_regularized_bootstrap_smoke_samples.csv"
...
OUT_RECON = DATA / "regularized_full_pysr_backreaction_smoke_reconstruction_vector.csv"
OUT_OMEGA = DATA / "regularized_full_pysr_backreaction_smoke_omega_vector.csv"
OUT_OMEGA_SAMPLES = DATA / "regularized_full_pysr_backreaction_smoke_omega_samples.csv"
OUT_COV = DATA / "regularized_full_pysr_backreaction_smoke_omega_covariance.csv"
OUT_SCORE = EVIDENCE / "regularized_full_pysr_backreaction_smoke_bridge_scorecard.csv"
OUT_ROW = EVIDENCE / "regularized_full_pysr_backreaction_smoke_bridge_row_audit.csv"
OUT_SUMMARY = EVIDENCE / "regularized_full_pysr_backreaction_smoke_summary.csv"
OUT_DOC = DOCS / "regularized_full_pysr_backreaction_smoke_null.md"

CLAIM_BOUNDARY = "regularized_full_pysr_backreaction_smoke_null_no_measurement_validation"
...
def score_bridge(omega_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for route in ROUTES:
        ...
        score_rows.append(
            {
                "AuditID": "REGULARIZED_FULL_PYSR_BACKREACTION_SMOKE_BRIDGE_V1",
        ...
        for i, vector_row in vector.iterrows():
            row_rows.append(
                {
                    "AuditID": "REGULARIZED_FULL_PYSR_BACKREACTION_SMOKE_ROW_V1",
                    "RouteID": route["RouteID"],
...
def main() -> None:
    ...
    summary = pd.DataFrame(
        [
            {
                "AuditID": "REGULARIZED_FULL_PYSR_BACKREACTION_SMOKE_NULL_V1",
                "RegularizedFullSmokeNullBuilt": True,
                ...
                "ScaleFitAllowed": False,
                "MeasurementValidationAllowed": False,
                "CurrentStatus": "REGULARIZED_FULL_PYSR_BACKREACTION_SMOKE_SCORED",
                "StrongestAllowedClaim": (
                    "a derivative-regularized D + H smoke backreaction null can be built and scored against locked K2"
                ),
                "PrimaryResidualRisk": (
                    "regularization is a smoke-scale D-branch governance rule and source-native covariance is still missing"
                ),
                "NextAction": "diagnose whether low-depth Omega sensitivity is reduced without changing locked K2",
                "ClaimBoundary": CLAIM_BOUNDARY,
            }
        ]
    )
    ...
    OUT_DOC.write_text(
        "\n".join(
            [
                "# Regularized Full PySR Backreaction Smoke Null",
                "",
                "Status: REGULARIZED_FULL_PYSR_BACKREACTION_SMOKE_SCORED.",
                "",
                "This artifact combines a derivative-regularized D branch with the normalized-PySR H_D branch and evaluates the fixed backreaction formula. It is not a measurement-validating physical-null benchmark.",
                "",
                "## Result",
                "",
