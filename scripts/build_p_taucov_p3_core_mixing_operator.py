#!/usr/bin/env python3
"""Build the target-blind P3 core-mixing projection-response operator packet."""
...
DOC = ROOT / "docs/p_taucov_p3_core_mixing_operator.md"
MANIFEST = ROOT / "evidence/p_taucov_p3_core_mixing_operator_manifest.yaml"
SHA256 = ROOT / "evidence/p_taucov_p3_core_mixing_operator.sha256"
METRICS = ROOT / "evidence/p_taucov_p3_core_mixing_operator_metrics.csv"
LEAKAGE = ROOT / "evidence/p_taucov_p3_core_mixing_operator_leakage_audit.csv"

PROTOCOL_ID = "P_TAUCOV_BRANCH_LOCALIZED_COVARIANCE_RESPONSE_v1"
PACKET_ID = "P_TAUCOV_P3_CORE_MIXING_OPERATOR_v1"

PARENT_AXIS = "PHI_PARENT_SOURCE"
BRANCH_AXIS = "B_BRANCH_RESPONSE"
...
def main() -> int:
    ...
    p_branch = idx[axis_to_id[BRANCH_AXIS]]
    p_projection = idx[axis_to_id[PROJECTION_AXIS]]
    p_morphology = idx[axis_to_id[MORPHOLOGY_AXIS]]

    p3 = np.zeros((len(ids), len(ids)), dtype=float)
    active = [p_parent, p_branch, p_morphology, p_projection]
    links = [(row, col) for row in active for col in active if row != col]
    weight = 1.0 / np.sqrt(float(len(links)))
    for row, col in links:
        p3[row, col] = weight

    p3_df = pd.DataFrame(p3, columns=ids)
    p3_df.insert(0, "coordinate_id", ids)
    p3_df.to_csv(P3, index=False)

    p_red = read_matrix(P_RED, ids)
    delta = p3 @ p3.T
    comm = p_red @ p3 - p3 @ p_red
    p3_fro = float(np.linalg.norm(p3, ord="fro"))
    ...
    metrics = pd.DataFrame(
        [
            ("P3_FROBENIUS_NORM", p3_fro, abs(p3_fro - 1.0) < 1e-12, "==1"),
            ("P3_COMMUTATOR_WITH_P_RED_FROBENIUS", comm_fro, comm_fro > 0.0, ">0"),
            ("P3_DELTA_DIAGONAL_SHARE", diag_share, diag_share < 0.95, "<0.95"),
            ("P3_DELTA_OFFDIAGONAL_SHARE", offdiag_share, offdiag_share >= 0.05, ">=0.05"),
            ("P3_EFFECTIVE_RANK_FRACTION", effective_rank_fraction, 0.10 <= effective_rank_fraction <= 0.85, "0.10..0.85"),
            ("P3_SUPPORT_ENTROPY", normalized_entropy, 0.25 <= normalized_entropy <= 0.85, "0.25..0.85"),
            ("P3_LABEL_PROXY_OVERLAP", label_proxy_overlap, label_proxy_overlap <= 0.35, "<=0.35"),
            ("P3_STRUCTURAL_NULL_MARGIN", null_margin, null_margin >= 0.05, ">=0.05"),
        ],
    ...
    manifest = {
        ...
        "ConstructionRule": "target_blind_core_mixing_over_four_tau_side_axes",
        "Reason": "P2 fixed null separation but failed support entropy because its covariance response remained too narrowly localized.",
        ...
        "ClaimBoundary": "p3_core_mixing_operator_frozen_no_model_packet_no_scoring",
    }
    MANIFEST.write_text(yaml.safe_dump(manifest, sort_keys=False), encoding="utf-8")
    SHA256.write_text(
        "\n".join(f"{file_sha256(path)}  {path.relative_to(ROOT)}" for path in [P3, MANIFEST, METRICS]) + "\n",
        encoding="utf-8",
    )
    ...
    leakage = pd.DataFrame(
        [
            {
                "AuditID": "P_TAUCOV_P3_CORE_MIXING_OPERATOR_LEAKAGE_AUDIT",
                "CheckID": check_id,
                "Passed": True,
                "Required": True,
                "Evidence": evidence,
    ...
    DOC.write_text(
        """# P-TauCov P3 Core-Mixing Operator

Status: target-blind core-mixing projection operator / no updated model packet / no
empirical scoring authorization.

The P2 operator fixed the P1 null-separation failure, but its covariance
response remained too narrowly localized and failed the support-entropy gate.
This P3 packet freezes a support-widening core-mixing structure without using
target residuals, P5C v3 outcomes, score behavior, or family-localized tuning.

## Frozen Construction

```text
all ordered off-diagonal links among:
  parent source
  branch response
  parent morphology
...
```text
each nonzero link = 1/sqrt(12)
```

The four-axis core is deliberately restricted to Tau-side coordinates and
excludes coordinate-convention and external-metadata axes. The design directly
targets the P2 support-entropy failure while preserving the no-outcome,
no-residual, no-score construction rule.
...
```text
A target-blind P3 core-mixing operator is frozen for later specificity
auditing.
```
...
```text
The P3 operator has produced a Tau signal, passed empirical scoring, or
...
""",
    ...
    for path in [P3, DOC, MANIFEST, SHA256, METRICS, LEAKAGE]:
        print(f"Wrote {path}")
    return 0
