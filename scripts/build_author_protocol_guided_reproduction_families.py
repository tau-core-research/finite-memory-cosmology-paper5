#!/usr/bin/env python3
"""Build author-protocol-guided local reproduction families.
...
exports are not available, so exact author-native reproduction remains closed.
"""
...
OUT_REGISTRY = EVIDENCE / "author_protocol_family_registry.csv"
OUT_RECON = DATA / "author_protocol_guided_reproduction_reconstruction_vector.csv"
OUT_META = DATA / "author_protocol_guided_reproduction_selection_metadata.csv"
OUT_OMEGA = DATA / "author_protocol_guided_reproduction_backreaction_vector.csv"
OUT_SAMPLES = DATA / "author_protocol_guided_reproduction_backreaction_bootstrap_samples.csv"
OUT_COV = DATA / "author_protocol_guided_reproduction_backreaction_covariance.csv"
OUT_SUMMARY = EVIDENCE / "author_protocol_guided_reproduction_summary.csv"
OUT_DOC = DOCS / "author_protocol_guided_reproduction_families.md"

CLAIM_BOUNDARY = "author_protocol_guided_reproduction_no_measurement_validation"
...
def build_registry() -> pd.DataFrame:
    rows = [
        {
            "FamilyID": "AUTHOR_DA_FILTERED_PROXY",
            ...
            "UsedInThisRun": True,
            "ResidualRisk": "manual shape rejection and author expression exports are unavailable",
            "ClaimBoundary": CLAIM_BOUNDARY,
        },
        {
            "FamilyID": "AUTHOR_HD_CRITERIA_1_SIMPLE_PROXY",
        ...
        {
            "FamilyID": "AUTHOR_HD_CRITERIA_2_LOW_LOSS_PROXY",
            ...
            "NumericalExportAvailable": True,
            "UsedInThisRun": True,
            "ResidualRisk": "manual nonpathological filtering is approximated, not author-native",
            "ClaimBoundary": CLAIM_BOUNDARY,
        },
        {
            "FamilyID": "AUTHOR_HD_CRITERIA_3_LOSS_COMPLEXITY",
            ...
            "NumericalExportAvailable": True,
            "UsedInThisRun": True,
            "ResidualRisk": "PySR/cp3-bench implementation details and author seeds may differ",
            "ClaimBoundary": CLAIM_BOUNDARY,
        },
        {
            "FamilyID": "AUTHOR_DESI_BRANCH_TARGET",
        ...
        {
            "FamilyID": "AUTHOR_BOSS_EBOSS_BRANCH_TARGET",
...
def family_masks(meta: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    return {
        "AUTHOR_HD_CRITERIA_1_SIMPLE_PROXY": da_filtered & simple,
        "AUTHOR_HD_CRITERIA_2_LOW_LOSS_PROXY": da_filtered & low_loss,
        "AUTHOR_HD_CRITERIA_3_LOSS_COMPLEXITY": da_filtered & criteria3,
    }
                ...
                rows.append(
                    {
                        ...
                        "Source": "author_protocol_guided_local_reproduction_bootstrap_covariance",
                        "ClaimBoundary": CLAIM_BOUNDARY,
                    }
                )
    return pd.DataFrame(rows)
...
def main() -> None:
                ...
                family_omega_samples.append(
                    {
                        ...
                        "Source": "author_protocol_guided_local_reproduction_bootstrap",
                        "ClaimBoundary": CLAIM_BOUNDARY,
                    }
                )
            ...
            recon_rows.append(
                {
                    ...
                    "Source": "author_protocol_guided_local_reproduction_median",
    ...
    summary = pd.DataFrame(
        [
            {
                "AuditID": "AUTHOR_PROTOCOL_GUIDED_REPRODUCTION_V1",
                ...
                "ExactAuthorNative": False,
                "MeasurementValidationAllowed": False,
                "CurrentStatus": "AUTHOR_PROTOCOL_GUIDED_LOCAL_REPRODUCTION_READY",
                "StrongestAllowedClaim": (
                    "published selection protocols can define local reproduction-family labels and proxy filters"
                ),
                "PrimaryResidualRisk": (
                    "manual hall-of-fame decisions and branch-specific author exports remain unavailable"
                ),
                "NextAction": (
                    "score author-protocol-guided families against locked K2 and separately implement DESI/eBOSS branch-specific reruns"
                ),
                "ClaimBoundary": CLAIM_BOUNDARY,
            }
        ]
    ...
    OUT_DOC.write_text(
        "\n".join(
            [
                "# Author-Protocol-Guided Reproduction Families",
                "",
                "Status: AUTHOR_PROTOCOL_GUIDED_LOCAL_REPRODUCTION_READY.",
                "",
                "These families use labels and selection logic derived from the published symbolic-reconstruction protocol, while the numerical curves are local 200-bootstrap reproductions. They are not author/source-native exports.",
                "",
                "## Available Families",
                "",
                "- `AUTHOR_HD_CRITERIA_1_SIMPLE_PROXY`",
                "- `AUTHOR_HD_CRITERIA_2_LOW_LOSS_PROXY`",
                "- `AUTHOR_HD_CRITERIA_3_LOSS_COMPLEXITY`",
                "",
                "## Registered But Not Yet Numerically Exported",
                "",
                "- `AUTHOR_DESI_BRANCH_TARGET`",
                "- `AUTHOR_BOSS_EBOSS_BRANCH_TARGET`",
                "",
                "## Outputs",
                "",
