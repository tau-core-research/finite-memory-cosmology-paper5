#!/usr/bin/env python3
"""Build and score a full normalized-PySR D+H smoke backreaction null."""
...
D_VECTOR = DATA / "source_native_normalized_criteria3_d_branch_bootstrap_smoke_reconstruction_vector.csv"
D_SAMPLES = DATA / "source_native_normalized_criteria3_d_branch_bootstrap_smoke_samples.csv"
...
OUT_RECON = DATA / "full_normalized_pysr_backreaction_smoke_reconstruction_vector.csv"
OUT_OMEGA = DATA / "full_normalized_pysr_backreaction_smoke_omega_vector.csv"
OUT_OMEGA_SAMPLES = DATA / "full_normalized_pysr_backreaction_smoke_omega_samples.csv"
OUT_COV = DATA / "full_normalized_pysr_backreaction_smoke_omega_covariance.csv"
OUT_SCORE = EVIDENCE / "full_normalized_pysr_backreaction_smoke_bridge_scorecard.csv"
OUT_ROW = EVIDENCE / "full_normalized_pysr_backreaction_smoke_bridge_row_audit.csv"
OUT_SUMMARY = EVIDENCE / "full_normalized_pysr_backreaction_smoke_summary.csv"
OUT_DOC = DOCS / "full_normalized_pysr_backreaction_smoke_null.md"

CLAIM_BOUNDARY = "full_normalized_pysr_backreaction_smoke_null_no_measurement_validation"
...
def build_reconstruction() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    recon = pd.DataFrame(
        {
            "FamilyID": "FULL_NORMALIZED_PYSR_D_AND_H_BRANCHES",
            "SampleID": 0,
            "z": z,
            "D": D,
            "D_prime": Dp,
            ...
            "H_D": H,
            "H_D_prime": Hp,
            "Source": "D_and_H_from_normalized_pysr_bootstrap_smoke",
            "SelectionRule": "full_normalized_pysr_smoke_no_K2_fit_no_K1_refit",
    ...
    omega_df = pd.DataFrame(
        {
            ...
            "Source": "full_normalized_pysr_backreaction_smoke_reconstruction_vector",
            "ClaimBoundary": CLAIM_BOUNDARY,
        }
    )
...
def score_bridge(omega_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for route in ROUTES:
        ...
        score_rows.append(
            {
                "AuditID": "FULL_NORMALIZED_PYSR_BACKREACTION_SMOKE_BRIDGE_V1",
        ...
        for i, vector_row in vector.iterrows():
            row_rows.append(
                {
                    "AuditID": "FULL_NORMALIZED_PYSR_BACKREACTION_SMOKE_ROW_V1",
                    "RouteID": route["RouteID"],
...
def main() -> None:
    ...
    summary = pd.DataFrame(
        [
            {
                "AuditID": "FULL_NORMALIZED_PYSR_BACKREACTION_SMOKE_NULL_V1",
                "FullSmokeNullBuilt": True,
                ...
                "DBranchNormalizedPySRSmoke": True,
                ...
                "ScaleFitAllowed": False,
                "MeasurementValidationAllowed": False,
                "CurrentStatus": "FULL_NORMALIZED_PYSR_BACKREACTION_SMOKE_SCORED",
                "StrongestAllowedClaim": (
                    "a full D+H normalized-PySR smoke backreaction null can be built and scored against locked K2"
                ),
                "PrimaryResidualRisk": (
                    "D-branch derivative sensitivity is high at low depth and the bootstrap is smoke-scale"
                ),
                "NextAction": (
                    "diagnose D-branch derivative sensitivity and then scale only stable selector settings to 200 bootstrap expressions"
                ),
                "ClaimBoundary": CLAIM_BOUNDARY,
            }
        ]
    ...
    OUT_DOC.write_text(
        "\n".join(
            [
                "# Full Normalized PySR Backreaction Smoke Null",
                "",
                "Status: FULL_NORMALIZED_PYSR_BACKREACTION_SMOKE_SCORED.",
                "",
                "This artifact combines normalized-PySR D and H_D bootstrap-smoke branches and evaluates the fixed backreaction formula. It is not the final source-native physical-null benchmark.",
                "",
                "## Result",
                "",
