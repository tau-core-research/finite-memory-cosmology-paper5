#!/usr/bin/env python3
"""Run the P-TauCov signed-response alignment scorecard."""
...
AUTH = ROOT / "evidence/p_taucov_signed_response_final_manifest.yaml"
...
OUT_OOS = ROOT / "evidence/p_taucov_signed_response_oos_scorecard.csv"
OUT_NULLS = ROOT / "evidence/p_taucov_signed_response_null_scorecard.csv"
OUT_SUMMARY = ROOT / "evidence/p_taucov_signed_response_scorecard_summary.csv"

PROTOCOL_ID = "P_TAUCOV_BRANCH_LOCALIZED_COVARIANCE_RESPONSE_v1"
AUDIT_ID = "P_TAUCOV_SIGNED_RESPONSE_ALIGNMENT_SCORECARD_v1"
KERNEL_ID = "P_TAUCOV_BRANCH_LOCALIZED_SIGNED_KERNEL"
CLAIM_BOUNDARY = "signed_response_alignment_scorecard_no_survival_claim"
...
def load_authorization() -> None:
    ...
    if auth.get("SignedResponseScoringAuthorized") is not True:
        raise RuntimeError("Signed-response scoring is not authorized.")
    if auth.get("AuthorizedScope") != "signed_response_alignment_scorecard_only":
        raise RuntimeError("Manifest scope does not match signed-response scorecard.")
...
def normalize_kernel(kernel: np.ndarray) -> np.ndarray:
    ...
    if fro == 0.0:
        raise RuntimeError("Zero signed kernel.")
    return kernel / fro
...
def evaluate_fold(p5c, rows: pd.DataFrame, kernel: np.ndarray, fold: pd.Series) -> dict | None:
    ...
    score = signed_statistic(resid_test, sigma2, k_test)
    return {
        "SignedS": score,
        "Sigma2": sigma2,
...
def null_kernels(kernel: np.ndarray) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "DIAGONAL_SIGNED_CONTROL": normalize_kernel(diag) if np.linalg.norm(diag, ord="fro") > 0 else diag,
...
def main() -> int:
    load_authorization()
    tau_ids, signed_tau = matrix_from_long(SIGNED_MAP)
    empirical_ids, bridge = load_bridge(tau_ids)
    signed_kernel = normalize_kernel(bridge @ signed_tau @ bridge.T)
    ...
    if empirical_ids != rows["RowID"].astype(str).tolist():
        raise RuntimeError("Bridge empirical row order does not match scorecard row order.")
    folds = pd.read_csv(p5c.FOLDS)
    oos_rows = []
    for _, fold in folds.iterrows():
        result = evaluate_fold(p5c, rows, signed_kernel, fold)
        if result is None:
            continue
        oos_rows.append(
            {
    ...
    null_rows = []
    for null_id, null_kernel in null_kernels(signed_kernel).items():
        null_oos = []
        for _, fold in folds.iterrows():
            result = evaluate_fold(p5c, rows, null_kernel, fold)
            if result is not None and bool(fold["PrimaryOOS"]):
                null_oos.append(float(result["SignedS"]))
        null_rows.append(
            {
                "ProtocolID": PROTOCOL_ID,
                "AuditID": AUDIT_ID,
                "NullID": null_id,
                "PrimarySignedS": float(np.sum(null_oos)),
                "ClaimBoundary": CLAIM_BOUNDARY,
            }
        )
    ...
    status = (
        "P_TAUCOV_SIGNED_RESPONSE_ALIGNMENT_POSITIVE_NO_SURVIVAL_CLAIM"
        if primary_s > required_null_max and family_s > 0.0 and clock_s > 0.0 and max_family_share <= 0.5
        else "P_TAUCOV_SIGNED_RESPONSE_ALIGNMENT_FAIL_NO_SURVIVAL_CLAIM"
    )
    pd.DataFrame(
        [
            {
                ...
                "SignedResponseScoringAuthorized": True,
