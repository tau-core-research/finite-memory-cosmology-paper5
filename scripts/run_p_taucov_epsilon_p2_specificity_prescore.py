#!/usr/bin/env python3
"""Run the target-blind epsilon_P2 P-TauCov specificity prescore audit."""
...
DOC = ROOT / "docs/p_taucov_epsilon_p2_specificity_prescore.md"
CSV = ROOT / "evidence/p_taucov_epsilon_p2_specificity_prescore.csv"
SUMMARY = ROOT / "evidence/p_taucov_epsilon_p2_specificity_prescore_summary.csv"
MODEL_PACKET = ROOT / "evidence/p_taucov_epsilon_p2_model_packet.yaml"
THRESHOLDS = ROOT / "evidence/p_taucov_linear_specificity_threshold_freeze.csv"

PROTOCOL_ID = "P_TAUCOV_BRANCH_LOCALIZED_COVARIANCE_RESPONSE_v1"
AUDIT_ID = "P_TAUCOV_EPSILON_P2_SPECIFICITY_AUDIT_v1"
RUN_ID = "P_TAUCOV_EPSILON_P2_SPECIFICITY_PRESCORE_RUN_v1"
...
def write_blocked(reason: str) -> int:
        ...
        rows.append(
            {
                ...
                "EpsilonP2CandidateFrozen": False,
                "ScoringAuthorized": False,
                "ClaimBoundary": "epsilon_p2_specificity_prescore_blocked_no_scoring",
            }
        )
    ...
    DOC.write_text(
        f"""# P-TauCov Epsilon-P2 Specificity Prescore
...
""",
    ...
    print("P_TAUCOV_EPSILON_P2_SPECIFICITY_PRESCORE_BLOCKED")
    return 0
...
def main() -> int:
    ...
    if not MODEL_PACKET.exists():
        return write_blocked("missing_epsilon_p2_model_packet")
    ...
    _, p1 = read_matrix(input_files["P2"])
        ...
        rows.append(
            {
                ...
                "EpsilonP2CandidateFrozen": False,
                "ScoringAuthorized": False,
                "ClaimBoundary": "epsilon_p2_specificity_prescore_evaluated_no_freeze_no_scoring",
            }
        )
    ...
    status = "PASS_NOT_FROZEN" if passed_count == len(df) else "FAIL_EPSILON_P2_NOT_SPECIFIC"
    next_step = "create_epsilon_p2_freeze_manifest" if status == "PASS_NOT_FROZEN" else "revise_target_blind_projection_support_operator"
    pd.DataFrame(
        [
            {
                "ProtocolID": PROTOCOL_ID,
...
""",
    ...
    print(f"P_TAUCOV_EPSILON_P2_SPECIFICITY_PRESCORE_{status}")
    return 0
