...
RECON = DATA / "regularized_full_pysr_backreaction_200_reconstruction_vector.csv"
OMEGA = DATA / "regularized_full_pysr_backreaction_200_omega_vector.csv"
ROW_AUDIT = EVIDENCE / "regularized_full_pysr_backreaction_200_bridge_row_audit.csv"

OUT_ROW = EVIDENCE / "regularized_full_pysr_backreaction_200_sensitivity_row.csv"
OUT_SUMMARY = EVIDENCE / "regularized_full_pysr_backreaction_200_sensitivity_summary.csv"
OUT_DOC = DOCS / "regularized_full_pysr_backreaction_200_sensitivity.md"

CLAIM_BOUNDARY = "regularized_full_pysr_backreaction_200_sensitivity_no_measurement_validation"
...
def main() -> None:
    ...
    summary = pd.DataFrame(
        [
            {
                "AuditID": "REGULARIZED_FULL_PYSR_BACKREACTION_200_SENSITIVITY_V1",
                "Rows": int(len(rows)),
                ...
                "CurrentStatus": "REGULARIZED_FULL_PYSR_BACKREACTION_200_SENSITIVITY_AUDITED",
                "StrongestAllowedClaim": (
                    "the derivative-regularized full PySR 200-bootstrap null has audited low-depth sensitivity"
                ),
                "PrimaryResidualRisk": (
                    "the result is 200-bootstrap preflight scale but does not replace source-native covariance"
                ),
                "NextAction": (
                    "compare against the 12-smoke regularized null and keep measurement validation closed"
                ),
    ...
    OUT_DOC.write_text(
        "\n".join(
            [
                "# Regularized Full PySR Backreaction 200 Sensitivity",
                "",
                "Status: REGULARIZED_FULL_PYSR_BACKREACTION_200_SENSITIVITY_AUDITED.",
                "",
                "The 200-bootstrap derivative-regularized D and H_D branches are scored through the same fixed backreaction formula. This is a sensitivity audit, not a measurement-validation result.",
                "",
                "## Key Numbers",
                "",
