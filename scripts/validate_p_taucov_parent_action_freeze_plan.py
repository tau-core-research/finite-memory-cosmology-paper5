...
def main() -> int:
    ...
    if all(path.exists() for path in [PLAN, SUMMARY, DOC]):
        ...
        add("at_least_one_step_completed", int(plan["Completed"].sum()) >= 1)
        add("scorecard_step_completed", bool(plan.loc[plan["FreezeStep"].eq("FREEZE_01_PRIMARY_SCORECARD_SCRIPT"), "Completed"].iloc[0]) is True)
