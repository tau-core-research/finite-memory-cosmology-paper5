...
def main() -> int:
    ...
    if all(path.exists() for path in [ROLLUP, SUMMARY, DOC]):
        ...
        add("expected_remaining_blockers", blockers == set())
        add("expected_resolved_nonblockers", {"ReferenceState", "L_B_red", "D_Phi_F_B", "D_B_M_proj", "CovarianceMap", "DynamicalStability"}.issubset(nonblockers))
        ...
        add("operational_reference_frozen_recorded", bool(summary["OperationalReferenceFrozen"]) is True)
        add("dynamical_assembly_ready_recorded", bool(summary["DynamicalStabilityAssemblyReady"]) is True)
