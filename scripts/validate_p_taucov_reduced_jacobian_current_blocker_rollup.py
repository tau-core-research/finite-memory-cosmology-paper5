...
def main() -> int:
    ...
    if all(path.exists() for path in [ROLLUP, SUMMARY, DOC]):
        ...
        add("expected_remaining_blockers", blockers == {"DynamicalStability"})
        add("expected_resolved_nonblockers", {"ReferenceState", "L_B_red", "D_Phi_F_B", "D_B_M_proj", "CovarianceMap"}.issubset(nonblockers))
