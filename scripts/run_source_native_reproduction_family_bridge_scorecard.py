#!/usr/bin/env python3
"""Score local reproduction-family exports against locked K2."""
...
OMEGA = DATA / "source_native_reproduction_family_backreaction_vector.csv"
OUT_ROW = EVIDENCE / "source_native_reproduction_family_bridge_row_audit.csv"
OUT_SCORE = EVIDENCE / "source_native_reproduction_family_bridge_scorecard.csv"
OUT_SUMMARY = EVIDENCE / "source_native_reproduction_family_bridge_summary.csv"
OUT_DOC = DOCS / "source_native_reproduction_family_bridge_scorecard.md"

CLAIM_BOUNDARY = "source_native_reproduction_family_bridge_no_measurement_validation"
...
def main() -> None:
    ...
    for route in ROUTES:
        ...
        for (family_id, sample_id), group in omega.groupby(["FamilyID", "SampleID"], sort=False):
            ...
            score_rows.append(
                {
                    "AuditID": "SOURCE_NATIVE_REPRODUCTION_FAMILY_BRIDGE_SCORECARD_V1",
            ...
            for i, vector_row in vector.iterrows():
                row_rows.append(
                    {
                        "AuditID": "SOURCE_NATIVE_REPRODUCTION_FAMILY_BRIDGE_ROW_V1",
                        "RouteID": route["RouteID"],
                        "FamilyID": family_id,
                        "SampleID": sample_id,
    ...
    best_family = (
        score.groupby("FamilyID")["FamilyBackreactionChi2"]
        .mean()
        .sort_values()
        .index[0]
    )
        ...
        [
            {
                ...
                "MeasurementValidationAllowed": False,
                "CurrentStatus": "REPRODUCTION_FAMILY_BRIDGE_SCORED_SOURCE_NATIVE_STILL_BLOCKED",
                "StrongestAllowedClaim": (
                    "local reproduction families can be scored against locked K2 as preflight comparators"
                ),
                "PrimaryResidualRisk": (
                    "local family rules are metadata-based reproductions, not author/source-native symbolic-regression exports"
                ),
                "NextAction": "audit row/zone dominance and compare family behavior to the single criteria-set-3 candidate",
                "ClaimBoundary": CLAIM_BOUNDARY,
            }
        ]
    )
    ...
    OUT_DOC.write_text(
        "\n".join(
            [
                "# Source-Native Reproduction Family Bridge Scorecard",
                "",
                f"Status: {summary.iloc[0]['CurrentStatus']}.",
                "",
                "This scorecard compares local reproduction families against locked K2. It does not use author exports, fit a scale, refit K1, or modify locked K2.",
                "",
                "## Key Numbers",
                "",
                ...
                "## Boundary",
                "",
                "This is a local reproduction-family preflight artifact only. It is not measurement validation.",
                "",
            ]
        ),
        encoding="utf-8",
