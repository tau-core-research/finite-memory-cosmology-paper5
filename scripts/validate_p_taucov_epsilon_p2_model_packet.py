#!/usr/bin/env python3
"""Validate the target-blind epsilon_P/P2 model packet."""
...
OUT = ROOT / "evidence/p_taucov_epsilon_p2_model_packet_validation.csv"
...
def main() -> int:
    ...
    def add(check_id: str, passed: bool, required: bool = True) -> None:
        records.append(
            {
                "AuditID": "P_TAUCOV_EPSILON_P2_MODEL_PACKET_VALIDATION",
                "CheckID": check_id,
                "Passed": bool(passed),
    ...
    if not all(path.exists() for path in required_paths):
        ...
        print("P_TAUCOV_EPSILON_P2_MODEL_PACKET_INVALID")
        return 1
    ...
    add("model_family_p2", manifest.get("ModelFamily") == "epsilon_P_projection_fork_response")
    add("lambda_B_zero", manifest.get("lambda_B") == 0)
    add("epsilon_P_one", manifest.get("epsilon_P") == 1)
    add("p1_failed", manifest.get("ParentP1Status") == "FAIL_EPSILON_P_NOT_SPECIFIC")
    add("p2_packet_frozen", manifest.get("EpsilonP2ModelPacketFrozen") is True)
    ...
    add("summary_packet_frozen", bool_from_csv(summary["EpsilonP2ModelPacketFrozen"].iloc[0]))
    ...
    if not failed.empty:
        ...
        return 1
    print("P_TAUCOV_EPSILON_P2_MODEL_PACKET_VALID_SPECIFICITY_ONLY_NO_SCORING")
    return 0
