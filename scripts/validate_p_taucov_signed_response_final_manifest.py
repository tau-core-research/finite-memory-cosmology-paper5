#!/usr/bin/env python3
"""Validate signed-response final manifest."""
...
OUT = ROOT / "evidence/p_taucov_signed_response_final_manifest_validation.csv"
...
def main() -> int:
    ...
    def add(check_id: str, passed: bool) -> None:
        records.append({"AuditID": "P_TAUCOV_SIGNED_RESPONSE_FINAL_MANIFEST_VALIDATION", "CheckID": check_id, "Passed": bool(passed), "Required": True, "Status": "PASS" if passed else "FAIL"})
    ...
    if all(path.exists() for path in [MANIFEST, SHA256, SUMMARY, DOC]):
        ...
        add("status_authorized", manifest.get("Status") == "P_TAUCOV_SIGNED_RESPONSE_ALIGNMENT_SCORECARD_AUTHORIZED_NO_SURVIVAL_CLAIM")
        add("summary_authorized", bool(summary["SignedResponseScoringAuthorized"]) is True)
        ...
        add("scope_signed_only", manifest.get("AuthorizedScope") == "signed_response_alignment_scorecard_only")
    ...
    print("P_TAUCOV_SIGNED_RESPONSE_FINAL_MANIFEST_VALID" if ok else "P_TAUCOV_SIGNED_RESPONSE_FINAL_MANIFEST_INVALID")
