#!/usr/bin/env python3
"""Build the target-blind epsilon_P P-TauCov model packet."""
...
DOC = ROOT / "docs/p_taucov_epsilon_p_model_packet.md"
MANIFEST = ROOT / "evidence/p_taucov_epsilon_p_model_packet.yaml"
SHA256 = ROOT / "evidence/p_taucov_epsilon_p_model_packet.sha256"
LEAKAGE = ROOT / "evidence/p_taucov_epsilon_p_model_packet_leakage_audit.csv"
SUMMARY = ROOT / "evidence/p_taucov_epsilon_p_model_packet_summary.csv"

PROTOCOL_ID = "P_TAUCOV_BRANCH_LOCALIZED_COVARIANCE_RESPONSE_v1"
PACKET_ID = "P_TAUCOV_EPSILON_P_MODEL_PACKET_v1_MINIMAL_P1"
...
INPUT_FILES = {
    ...
    "P1": "data/p_taucov/linear/P1_projection_response_operator.csv",
    "strict_linear_packet": "evidence/p_taucov_linear_model_packet.yaml",
    "strict_linear_prescore": "evidence/p_taucov_linear_specificity_prescore_summary.csv",
    "minimal_nonzero_route_freeze": "evidence/p_taucov_minimal_nonzero_route_freeze_summary.csv",
    "p1_manifest": "evidence/p_taucov_p1_projection_response_operator_manifest.yaml",
}
...
def main() -> int:
    ...
    if missing:
        raise FileNotFoundError("Missing epsilon_P packet inputs: " + ", ".join(missing))

    hashes = {key: file_sha256(ROOT / rel) for key, rel in INPUT_FILES.items()}
    manifest = {
        "ProtocolID": PROTOCOL_ID,
        "PacketID": PACKET_ID,
        "PacketType": "epsilon_p_model_packet",
        "ModelFamily": "epsilon_P_minimal_projection_response",
        "lambda_B": 0,
        "epsilon_P": 1,
        "EpsilonPAmplitudePolicy": "unit_amplitude_because_P1_has_unit_frobenius_norm",
        "ProjectionMap": "P_epsilon = P0 + epsilon_P * P1",
        "StrictLinearParentStatus": "FAIL_STRICT_LINEAR_REJECTED",
        "EpsilonPModelPacketFrozen": True,
        ...
        "ClaimBoundary": "epsilon_p_model_packet_authorizes_specificity_metrics_only_no_scoring",
    ...
    leakage = pd.DataFrame(
        [
            {
                "AuditID": "P_TAUCOV_EPSILON_P_MODEL_PACKET_LEAKAGE_AUDIT",
                "CheckID": check_id,
                "Passed": True,
                "Required": True,
                "Evidence": evidence,
            ...
            for check_id, evidence in [
                ...
                ("epsilon_amplitude_target_blind", "epsilon_P=1 follows unit-norm P1 convention, not score fitting"),
            ]
        ]
    )
        ...
        [
            {
                ...
                "MetricEvaluationAuthorized": True,
                "PTauCovScoringAuthorized": False,
                "NextStep": "run_epsilon_p_specificity_prescore",
