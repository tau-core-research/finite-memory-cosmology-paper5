#!/usr/bin/env python3
"""Run a D-branch bootstrap smoke with derivative regularity governance."""
...
OUT_BINNED = DATA / "d_branch_derivative_regularized_binned_training.csv"
OUT_SELECTIONS = DATA / "d_branch_derivative_regularized_bootstrap_smoke_selection_metadata.csv"
OUT_SAMPLES = DATA / "d_branch_derivative_regularized_bootstrap_smoke_samples.csv"
OUT_VECTOR = DATA / "d_branch_derivative_regularized_bootstrap_smoke_reconstruction_vector.csv"
OUT_COV = DATA / "d_branch_derivative_regularized_bootstrap_smoke_covariance.csv"
OUT_SUMMARY = EVIDENCE / "d_branch_derivative_regularized_bootstrap_smoke_summary.csv"
OUT_DOC = DOCS / "d_branch_derivative_regularized_bootstrap_smoke.md"

CLAIM_BOUNDARY = "d_branch_derivative_regularized_bootstrap_smoke_no_measurement_validation"
...
def candidate_regularized_scores(hof: pd.DataFrame, n_eff: int, z_grid: np.ndarray) -> pd.DataFrame:
    ...
    for idx, row in tmp.iterrows():
        try:
            ...
            regularity_penalty = low_excess
            regularized_score = float(row["NormalizedCriteria3Score"]) + LAMBDA_REGULARIZATION * regularity_penalty
        except Exception:
            expr = ""
            finite = False
            low_metric = float("inf")
            midhigh_metric = float("inf")
            regularity_penalty = float("inf")
        ...
        rows.append(
            {
                ...
                "DerivativeRegularityPenalty": regularity_penalty,
                "DerivativeRegularizedScore": regularized_score,
            }
        )
    scored = tmp.reset_index(drop=False).rename(columns={"index": "CandidateIndex"}).merge(
        pd.DataFrame(rows), on="CandidateIndex", how="left"
    )
...
def main() -> None:
    ...
    for run_idx in range(bootstrap_runs):
        ...
        hof = fit_hof(z_train, y_train, sigma_train, seed=401 + run_idx)
        ...
        selection_rows.append(
            {
                "RouteID": "D_BRANCH_DERIVATIVE_REGULARIZED_BOOTSTRAP_SMOKE_V1",
    ...
    vector["SelectionRule"] = "derivative_regularized_normalized_criteria3_no_K2_fit"
    ...
    summary = pd.DataFrame(
        [
            {
                "AuditID": "D_BRANCH_DERIVATIVE_REGULARIZED_BOOTSTRAP_SMOKE_V1",
                ...
                "AmplitudeFitUsed": False,
                "MeasurementValidationAllowed": False,
                "CurrentStatus": "D_BRANCH_DERIVATIVE_REGULARIZED_BOOTSTRAP_SMOKE_EXECUTED_NOT_FULL_SCALE",
                "StrongestAllowedClaim": (
                    "the D-branch smoke selector can export derivative-regularized D,D_prime,D_double_prime samples"
                ),
                "PrimaryResidualRisk": (
                    "this is still a smoke-scale binned SN proxy and not a source-native full bootstrap"
                ),
                "NextAction": (
                    "combine the regularized D branch with the existing H_D branch through the backreaction formula"
                ),
                "ClaimBoundary": CLAIM_BOUNDARY,
            }
        ]
    ...
    OUT_DOC.write_text(
        "\n".join(
            [
                "# D-Branch Derivative-Regularized Bootstrap Smoke",
                "",
                "Status: D_BRANCH_DERIVATIVE_REGULARIZED_BOOTSTRAP_SMOKE_EXECUTED_NOT_FULL_SCALE.",
                "",
                "This smoke run applies a D-branch-only derivative regularity guard on top of the normalized criteria-set-3 selector. It does not use K2, K1, target signs, or amplitude fitting.",
                "",
                "## Result",
                "",
                ...
                f"- Median mid/high curvature metric: {float(selections['MidHighCurvatureMetric'].median())}",
