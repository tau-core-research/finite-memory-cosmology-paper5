#!/usr/bin/env python3
"""Validate expanded parent-operator final manifest."""
...
OUT = ROOT / "evidence/p_taucov_expanded_parent_operator_final_manifest_validation.csv"

AUDIT_ID = "P_TAUCOV_EXPANDED_PARENT_OPERATOR_FINAL_MANIFEST_VALIDATION"
...
def main() -> int:
    ...
    if all(path.exists() for path in [MANIFEST, SHA256, SUMMARY, DOC]):
        ...
        add("status_authorized_no_survival", str(summary["Status"]) == "P_TAUCOV_EXPANDED_PARENT_OPERATOR_PRIMARY_SCORECARD_AUTHORIZED_NO_SURVIVAL_CLAIM")
        ...
        add("scope_primary_only", str(summary["AuthorizedScope"]) == "expanded_parent_operator_primary_covariance_scorecard_only")
        add("input_statuses_present", len(input_status) >= 9)
        add("input_hashes_present", len(input_hash) >= 9)
        add("required_hashes_present", {"candidate", "source_matrix", "domain_coordinates", "scorecard_script"}.issubset(set(required_hash)))
        add("manifest_sha_file_points_to_manifest", "p_taucov_expanded_parent_operator_final_manifest.yaml" in SHA256.read_text(encoding="utf-8"))
    ...
    print("P_TAUCOV_EXPANDED_PARENT_OPERATOR_FINAL_MANIFEST_VALID" if ok else "P_TAUCOV_EXPANDED_PARENT_OPERATOR_FINAL_MANIFEST_INVALID")
