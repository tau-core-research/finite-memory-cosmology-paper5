...
\section*{Finite-Memory Projection Corrections In Cosmological Consistency Diagnostics}
...
projection response over normalized depth, \$W(x)=1+\textbackslash{}rho x\textasciicircum{}3\$, with a passive
memory-budget bound \$\textbackslash{}rho \textbackslash{}leq 4\$. The construction is not presented as a complete
...
\$\$
0 \textbackslash{}leq x \textbackslash{}leq 1 .
\$\$
...
\$\$
W(x) = 1 + \textbackslash{}rho x\textasciicircum{}3 .
\$\$
...
\$\$
x = \textbackslash{}chi(z)/\textbackslash{}chi(z\_\{\textbackslash{}max\}),
\$\$

where \$\textbackslash{}chi\$ is a comoving-distance coordinate under a frozen reference
...
The baseline shape \$K\_1(x)\$ is a frozen diagnostic baseline imported from the
...
fixed diagnostic baseline. It does not establish that \$K\_1(x)\$ is unique,
physically final, or likelihood-preferred.

Several assumptions are therefore intentionally modest:

\noindent $\bullet$ the coordinate \$x\$ is a diagnostic ordering variable, not a new directly\\
  measured cosmological coordinate;
\noindent $\bullet$ the operator \$W(x)\$ is an effective response, not a complete field equation;\\
\noindent $\bullet$ the reconstructed envelopes are treated as diagnostic targets, not as final\\
  likelihood surfaces;
\noindent $\bullet$ the public module does not require the reader to accept any broader\\
  background theory.
...
\$\$
E\_\{\textbackslash{}mathrm\{tail\}\} = \textbackslash{}int\_0\textasciicircum{}1 \textbackslash{}rho x\textasciicircum{}3\textbackslash{},dx = \textbackslash{}frac\{\textbackslash{}rho\}\{4\}.
\$\$

The unit baseline budget is

\$\$
E\_0 = \textbackslash{}int\_0\textasciicircum{}1 1\textbackslash{},dx = 1.
\$\$

Passivity requires

\$\$
E\_\{\textbackslash{}mathrm\{tail\}\} \textbackslash{}leq E\_0,\textbackslash{}qquad
\textbackslash{}frac\{\textbackslash{}rho\}\{4\} \textbackslash{}leq 1,\textbackslash{}qquad
\textbackslash{}rho \textbackslash{}leq 4 .
\$\$
...
comparison requires \$\textbackslash{}rho > 4\$ in order to remain compatible with the target
...
\$\$
W\_p(x) = 1 + \textbackslash{}rho x\textasciicircum{}p .
\$\$

For this family, the passive memory rule generalizes to

\$\$
\textbackslash{}int\_0\textasciicircum{}1 \textbackslash{}rho x\textasciicircum{}p\textbackslash{},dx \textbackslash{}leq \textbackslash{}int\_0\textasciicircum{}1 1\textbackslash{},dx,\textbackslash{}qquad
\textbackslash{}frac\{\textbackslash{}rho\}\{p+1\} \textbackslash{}leq 1,\textbackslash{}qquad
\textbackslash{}rho \textbackslash{}leq p+1 .
\$\$

Two additional effective-operator rules select the usable shape:

\$\$
\textbackslash{}Delta W\_p(0.25) = (p+1)0.25\textasciicircum{}p \textbackslash{}leq 0.10 ,
\$\$

\$\$
\textbackslash{}int\_\{0.75\}\textasciicircum{}\{1\} (p+1)x\textasciicircum{}p\textbackslash{},dx \textbackslash{}leq 0.75 .
\$\$
...
\$\$
W(x) = 1 + \textbackslash{}rho x\textasciicircum{}3,\textbackslash{}qquad
\textbackslash{}eta \textbackslash{}in [0.25,0.50],\textbackslash{}qquad
\textbackslash{}rho \textbackslash{}in [3.00,4.00].
\$\$
...
|---|---|---|---|---|
| baseline | \$\textbackslash{}Delta W\_p(0.25)\textbackslash{}leq 0.10\$ | endpoint \$\textbackslash{}leq 0.75\$ | \$p=3\$ | cubic is the current audit-selected representative |
| relaxed low-depth | \$\textbackslash{}Delta W\_p(0.25)\textbackslash{}leq 0.15\$ | endpoint \$\textbackslash{}leq 0.75\$ | \$p=2,3\$ | more low-depth visibility broadens the set |
| strict low-depth | \$\textbackslash{}Delta W\_p(0.25)\textbackslash{}leq 0.075\$ | endpoint \$\textbackslash{}leq 0.75\$ | \$p=3\$ | cubic remains in this simple scan |
| relaxed endpoint | \$\textbackslash{}Delta W\_p(0.25)\textbackslash{}leq 0.10\$ | endpoint \$\textbackslash{}leq 0.85\$ | \$p=3,4\$ | endpoint relaxation admits a later kernel |
| strict endpoint | \$\textbackslash{}Delta W\_p(0.25)\textbackslash{}leq 0.10\$ | endpoint \$\textbackslash{}leq 0.60\$ | none | the simple power-kernel family can be emptied |
...
Operationally, let \$y\_i\$ denote the reconstructed diagnostic median at the
tested point \$i\$, and let \$[L\_i,U\_i]\$ denote the corresponding reconstructed
diagnostic envelope. A locked prediction \$\textbackslash{}hat y\_i\$ is envelope-compatible
when

\$\$
L\_i \textbackslash{}leq \textbackslash{}hat y\_i \textbackslash{}leq U\_i .
\$\$

If a diagonal diagnostic width \$\textbackslash{}sigma\_i\$ is available, the normalized residual
gate is

\$\$
\textbackslash{}left|\textbackslash{}hat y\_i-y\_i\textbackslash{}right|/\textbackslash{}sigma\_i \textbackslash{}leq 1 .
\$\$
...
the provisional mapping \$x=z/z\_\{\textbackslash{}max\}\$ over this grid.
...
\subsection*{9. Toward A Measurement Gate}
...
under the current diagnostic gate. A flat-LCDM \$\textbackslash{}chi(z)\$-normalized
...
bounded \$\textbackslash{}rho \textbackslash{}leq 4\$ scan shows that the \$\textbackslash{}chi\$-normalized operator-only
mapping has non-violating points inside the paper's locked \$\textbackslash{}rho\$ range. The
...
fixed \$\textbackslash{}rho=4\$ K2 prediction is non-violating on most tested coordinate
mappings, and the bounded \$\textbackslash{}rho\textbackslash{}in[3,4]\$ grid is non-violating across the
...
data/reconstruction\_families/source\_split\_reconstruction\_family\_responses.csv.
...
not\_marked\_primary\_candidate. This is intentional: it prepares the next
...
correlation length 0.25, and rho\_SN\_BAO=0 as the primary cross-covariance
...
supportive\_but\_narrow, not measurement validation.
...
response\_i=(1-alpha)*unit\_norm(centered[x\textasciicircum{}2]). The amplitude is the
...
\noindent $\bullet$ the BAO-only envelope overlap disappeared under the same locked K2 window;\\
\noindent $\bullet$ sign-stable SN+BAO points required the opposite sign from the locked\\
  prediction;
\noindent $\bullet$ compatibility required \$\textbackslash{}rho > 4\$ or a different kernel power chosen after\\
  seeing the envelope;
\noindent $\bullet$ the localized SN+BAO warning became sign-stable under a stronger\\
  reconstruction audit;
\noindent $\bullet$ a full covariance likelihood rejected the whole locked window rather than\\
...
\noindent $\bullet$ it gives a compact finite-memory operator whose amplitude is bounded before\\
  comparison with data;
\noindent $\bullet$ it interfaces naturally with FLRW consistency, optical-propagation, and\\
  backreaction tests;
\noindent $\bullet$ it demonstrates a way to handle sign-unstable reconstructed diagnostics\\
  without silently overclaiming;
\noindent $\bullet$ it provides a small public module that can be tested independently as a\\
  diagnostic proposal.
...
\noindent $\bullet$ a full covariance likelihood;\\
\noindent $\bullet$ a direct measurement claim;\\
\noindent $\bullet$ exclusion of cosmic backreaction;\\
\noindent $\bullet$ a complete derivation of cosmology from an underlying action;\\
\noindent $\bullet$ reconstruction of a full background theory.\\
...
\noindent $\bullet$ covariance-aware likelihood ingestion;\\
\noindent $\bullet$ null comparators for no-memory, smooth-response, optical-propagation, and\\
  backreaction-only alternatives;
\noindent $\bullet$ a public reconstruction benchmark;\\
\noindent $\bullet$ coordinate-mapping robustness across redshift-normalized, comoving-distance,\\
  optical-depth-like, and likelihood-native coordinates;
\noindent $\bullet$ direct comparison against BAO-only and SN+BAO reconstruction families;\\
\noindent $\bullet$ a frozen benchmark challenge in which the K2 window is locked before testing.\\
...
\$W(x)=1+\textbackslash{}rho x\textasciicircum{}3\$ follows from a passive memory budget and a simple
shape-selection audit over power kernels. With \$\textbackslash{}rho \textbackslash{}leq 4\$, the operator gives
...
\noindent $\bullet$ `evidence/result\_summary.csv`;\\
\noindent $\bullet$ `evidence/claim\_matrix.csv`;\\
\noindent $\bullet$ `evidence/diagnostic\_point\_audit.csv`;\\
\noindent $\bullet$ `evidence/source\_packet\_manifest.csv`;\\
\noindent $\bullet$ `evidence/coordinate\_mapping\_policy.csv`;\\
\noindent $\bullet$ `evidence/threshold\_sensitivity.csv`;\\
\noindent $\bullet$ `evidence/threshold\_kernel\_outcomes.csv`;\\
\noindent $\bullet$ `evidence/open\_problem\_resolution\_matrix.csv`.\\
\noindent $\bullet$ `docs/measurement\_gate\_plan.md`;\\
\noindent $\bullet$ `evidence/measurement\_gate\_matrix.csv`;\\
\noindent $\bullet$ `evidence/null\_model\_registry.csv`.\\
\noindent $\bullet$ `evidence/gate\_results\_current.csv`.\\
\noindent $\bullet$ `evidence/coordinate\_robustness\_results.csv`.\\
\noindent $\bullet$ `evidence/coordinate\_tension\_audit.csv`.\\
\noindent $\bullet$ `evidence/rho\_coordinate\_scan.csv`.\\
\noindent $\bullet$ `evidence/coordinate\_mapping\_geometry.csv`.\\
\noindent $\bullet$ `evidence/null\_comparison\_results.csv`.\\
\noindent $\bullet$ `evidence/model\_scorecard.csv`.\\
\noindent $\bullet$ `evidence/null\_dominance\_audit.csv`.\\
\noindent $\bullet$ `evidence/null\_dominance\_summary.csv`.\\
\noindent $\bullet$ `evidence/subset\_model\_scorecard.csv`.\\
\noindent $\bullet$ `evidence/cross\_validation\_results.csv`.\\
\noindent $\bullet$ `evidence/shrinkage\_covariance\_sensitivity.csv`.\\
\noindent $\bullet$ `evidence/k1\_baseline\_provenance\_audit.csv`.\\
\noindent $\bullet$ `evidence/public\_benchmark\_readiness.csv`.\\
\noindent $\bullet$ `docs/bao\_branch\_pivot\_decision.md`.\\
\noindent $\bullet$ `evidence/bao\_branch\_decision\_matrix.csv`.\\
\noindent $\bullet$ `docs/source\_split\_benchmark\_plan.md`.\\
\noindent $\bullet$ `evidence/source\_split\_readiness.csv`.\\
\noindent $\bullet$ `docs/source\_split\_transform\_contract.md`.\\
\noindent $\bullet$ `evidence/source\_split\_transform\_registry.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_transform\_readiness.csv`.\\
\noindent $\bullet$ `docs/sn\_residual\_transform\_preflight.md`.\\
\noindent $\bullet$ `evidence/sn\_residual\_preflight.csv`.\\
\noindent $\bullet$ `evidence/sn\_residual\_binned\_preflight.csv`.\\
\noindent $\bullet$ `evidence/sn\_residual\_preflight\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_joint\_preflight.md`.\\
\noindent $\bullet$ `evidence/source\_split\_joint\_preflight.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_joint\_preflight\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_standardized\_preflight.md`.\\
\noindent $\bullet$ `evidence/source\_split\_standardized\_preflight.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_standardized\_preflight\_summary.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_sign\_tension\_audit.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_sign\_tension\_summary.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_covariance\_sensitivity.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_covariance\_sensitivity\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_k1\_target\_plan.md`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_target\_registry.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_target\_readiness.csv`.\\
\noindent $\bullet$ `docs/source\_split\_covariance\_plan.md`.\\
\noindent $\bullet$ `evidence/source\_split\_covariance\_registry.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_covariance\_readiness.csv`.\\
\noindent $\bullet$ `docs/sign\_family\_export\_plan.md`.\\
\noindent $\bullet$ `evidence/sign\_family\_export\_registry.csv`.\\
\noindent $\bullet$ `evidence/sign\_family\_export\_readiness.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_gate\_dashboard.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_gate\_dashboard\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_export\_task\_queue.md`.\\
\noindent $\bullet$ `evidence/source\_split\_export\_task\_queue.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_export\_task\_queue\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_reconstruction\_family\_sources.md`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_source\_registry.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_source\_readiness.csv`.\\
\noindent $\bullet$ `docs/source\_split\_reconstruction\_family\_export\_schema.md`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_export\_schema.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_export\_template.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_export\_validation.csv`.\\
\noindent $\bullet$ `docs/source\_split\_reconstruction\_family\_candidate\_plan.md`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_candidate\_plan.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_candidate\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_reconstruction\_family\_response\_preview.md`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_response\_preview.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_reconstruction\_family\_response\_preview\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_family\_sign\_rule\_preview.md`.\\
\noindent $\bullet$ `evidence/source\_split\_family\_sign\_rule\_preview.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_family\_sign\_rule\_preview\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_sign\_rule\_promotion\_readiness.md`.\\
\noindent $\bullet$ `evidence/source\_split\_sign\_rule\_promotion\_readiness.csv`.\\
\noindent $\bullet$ `docs/source\_split\_k2\_scoring\_authorization.md`.\\
\noindent $\bullet$ `evidence/source\_split\_k2\_scoring\_authorization.csv`.\\
\noindent $\bullet$ `docs/source\_split\_candidate\_export\_handoff.md`.\\
\noindent $\bullet$ `evidence/source\_split\_candidate\_export\_handoff\_manifest.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_candidate\_export\_handoff\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_candidate\_path\_guard.md`.\\
\noindent $\bullet$ `evidence/source\_split\_candidate\_path\_guard.csv`.\\
\noindent $\bullet$ `docs/source\_split\_blocker\_matrix.md`.\\
\noindent $\bullet$ `evidence/source\_split\_blocker\_matrix.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_blocker\_matrix\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_sn\_branch\_export\_handoff.md`.\\
\noindent $\bullet$ `evidence/source\_split\_sn\_branch\_export\_handoff.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_sn\_branch\_export\_handoff\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_bao\_branch\_export\_handoff.md`.\\
\noindent $\bullet$ `evidence/source\_split\_bao\_branch\_export\_handoff.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_bao\_branch\_export\_handoff\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_reconstruction\_family\_candidate\_export.md`.\\
\noindent $\bullet$ `data/reconstruction\_families/source\_split\_reconstruction\_family\_responses.csv`.\\
\noindent $\bullet$ `docs/source\_split\_k2\_null\_scorecard.md`.\\
\noindent $\bullet$ `evidence/source\_split\_k2\_null\_scorecard.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_k2\_null\_scorecard\_summary.csv`.\\
\noindent $\bullet$ `docs/source\_split\_k1\_degeneracy\_audit.md`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_degeneracy\_audit.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_degeneracy\_summary.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_response\_requirements.csv`.\\
\noindent $\bullet$ `docs/source\_split\_k1\_candidate\_sensitivity.md`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_response\_candidate\_audit.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_response\_candidate\_summary.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_candidate\_sensitivity.csv`.\\
\noindent $\bullet$ `evidence/source\_split\_k1\_candidate\_sensitivity\_summary.csv`.\\
