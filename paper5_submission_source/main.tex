...
\noindent $\bullet$ a likelihood-native K1/no-memory target;\\
\noindent $\bullet$ a joint SN+BAO covariance or a preregistered shrinkage-covariance benchmark;\\
\noindent $\bullet$ calibrated optical and backreaction physical-null comparators;\\
\noindent $\bullet$ source-native or independently reproduced derivative/covariance exports for\\
  the backreaction route;
\noindent $\bullet$ a frozen public benchmark in which no K2 kernel, rho bound, or K1 baseline is\\
...
\noindent $\bullet$ `paper5\_submission\_source/`: paper-facing draft, LaTeX facsimile, and PDF;\\
\noindent $\bullet$ `src/fmc/`: scoring, covariance, coordinate, operator, and backreaction\\
  helper code;
\noindent $\bullet$ `scripts/`: executable reproduction and audit scripts;\\
\noindent $\bullet$ `data/`: derived public-input and local reproduction packets;\\
\noindent $\bullet$ `evidence/`: compact CSV outputs used by the manuscript and audit gates;\\
\noindent $\bullet$ `docs/measurement\_gate\_audit\_log.md`: detailed implementation/audit log moved\\
  out of the main paper;
\noindent $\bullet$ `frozen/`: locked operator and configuration artifacts;\\
\noindent $\bullet$ `tests/`: public reproducibility checks.\\

The key manuscript-facing outputs are:

\noindent $\bullet$ `evidence/gate\_results\_current.csv`;\\
\noindent $\bullet$ `evidence/tau\_core\_a2\_preflight\_proof\_packet\_summary.csv`;\\
\noindent $\bullet$ `evidence/author\_protocol\_guided\_dominance\_summary.csv`;\\
\noindent $\bullet$ `evidence/source\_native\_reproduction\_family\_dominance\_summary.csv`;\\
\noindent $\bullet$ `finite\_memory\_projection\_corrections.pdf`;\\
\noindent $\bullet$ `arxiv\_submission\_source.zip`.\\
